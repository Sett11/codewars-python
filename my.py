...
class Go:
    def __init__(self,n,m=None):
        ...
        self.board_states=deque([(deepcopy(self.game_board),self.g.copy(),self.current_move)])
    ...
    def inner_check_liberties(self,a):
        ...
        for i in a:
            ...
            for j,_ in i:
                x,y=j
                t=[(q,w) for q,w in [(x+1,y),(x-1,y),(x,y+1),(x,y-1)] if 0<=q<self.height and 0<=w<self.width and self.game_board[q][w]=='.']
                if not t:
                    c+=1
            if c==len(i):
                r.append(i)
    ...
    def move(self,*moves):
        ...
        for i,j in moves:
            ...
            curr_pos=self.game_board[k][p]
            if curr_pos=='.':
                self.current_move=((k,p),self.turn)
                self.game_board[k][p]=self.stones[self.turn]
                test=self.check_and_change(self.current_move)
                self.turn='black' if self.turn=='white' else 'white'
                self.board_states.append((deepcopy(self.game_board),self.g.copy(),self.current_move))
                if test is False:
                    self.stack.append('sui')
                    self.rollback()
        ...
        if self.board_states[-1][-1] and len(self.board_states)>2:
            x,y=(self.game_board,self.current_move),self.board_states[-3]
            if x==(y[0],y[-1]):
                self.rollback()
                raise Exception('Repeated move!')
            
    def get_position(self,s):
        return self.game_board[self.a.index(s[:-1])][self.s.index(s[-1])]
    ...
    def handicap_stones(self,n):
        ...
        while c<n:
            i,j=curr_han[c]
            self.game_board[i][j]=self.stones[self.turn]
            c+=1
        self.board_states.append((deepcopy(self.game_board),self.g.copy(),self.current_move))

    def rollback(self,n=1):
        self.stack.append(f'roll {n}')
        if self.res or n>self.count_moves or n>=len(self.board_states):
            raise Exception('Not rollback!')
        while n and self.count_moves:
            self.board_states.pop()
            t=self.board_states[-1]
            self.game_board,self.g,self.current_move=deepcopy(t[0]),t[1].copy(),t[2]
            self.turn='black' if self.turn=='white' else 'white'
            n-=1
            self.count_moves-=1
...
a=[('6E',), ('5B',), ('7A',), ('3C',), 'pass', ('5D',), ('5C',), 'roll 1', ('2E',), ('3D',), ('4E',), ('2F',), ('3F',), ('4A',), ('4H',), ('7F',), 'roll 12', ('1D',), ('4D',), ('2C',), ('4C',), ('3A',), ('7G',), ('6D',), ('4B',), ('6C',), ('4F',), ('1F',), ('3G',), ('5F',), ('1E',), ('3E',), ('7B',), ('5A',), ('2H',), ('2D',), ('1G',), ('6A',), ('4G',), ('2A',), 'roll 5', ('1A',), ('1C',), ('2B',), ('7H',), ('3B',), ('5H',), ('6B',), 'pass', ('7E',), ('7C',), ('3H',), ('1B',), 'sui', 'roll 1', ('1H',), ('7D',)]

for i in a:
    if isinstance(i,str):
        if i.startswith('roll'):
            game.rollback(int(i.split()[1]))
        if i=='sui':
            pass
        else:
            game.pass_turn()
